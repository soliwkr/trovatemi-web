...
export const GET: APIRoute = async ({ url }) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(JSON.stringify({ event: 'places.search.failed', status: response.status }));
      return json({ error: 'places_upstream_failed' }, 502);
    }

    const payload = await response.json();
    return json({ places: normalizeGoogleSearchResponse(payload), source: 'google_places' });
  } catch (error) {
