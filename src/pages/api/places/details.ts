...
export const GET: APIRoute = async ({ url }) => {
  ...
  if (!isValidPlaceId(placeId)) return json({ error: 'invalid_place_id' }, 400);
  ...
  try {
    const response = await fetch('https://places.googleapis.com/v1/places/' + encodeURIComponent(placeId), {
      headers: {
        ...
        'X-Go-FieldMask': googlePlaceDetailsFieldMask,
      },
    });

    if (!response.ok) {
      console.error(JSON.stringify({ event: 'places.details.failed', status: response.status }));
    ...
    if (!business.id || !business.name) return json({ error: 'place_not_found' }, 404);

    return json({ business, source: 'google_places' });
  } catch (error) {
