(() => {
  ...
  const patchCapture = () => {
    ...
    const businessName = document.querySelector('.business-confirmation h2')?.textContent?.trim();
    if (businessName) {
      setText('.capture-copy > p:not(.eyebrow)', `Ti preparo le 3 mosse in ordine per ${businessName}. Zero teoria: prima, seconda, terza.`);
    } else {
